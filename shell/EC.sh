...
#选项
printf "%*s\n" $(((${#menu_line}+$menu_title_len)/2)) "$menu_title"
echo $menu_line
echo "============================"
echo "    1. 删除虚拟设备信息"
echo "    2. 安装指定icqq版本"
...
#套用xdm的脚本，咱也不看直接套
clear
echo -e "$Y本方法是安装0.3.1版本然后使用协议6$0"
echo -e "$Q也就是安卓8.8.8$0"
echo -e "$R此方法必须完全删除虚拟设备信息！$0"
echo -e "$Q等风来就是不要脸，直接套人家脚本，诶嘿~"
echo -e "$Y5秒后开始安装$0"
sleep 5
...
if [ -d /root/Yunzai-Bot/data/icqq ]; then
   ...
   rm -rf data/device.json
    ...
    4)
#摘抄某人的原话
        echo -e "$L说简单点就是换号解决$0"
        echo -e "$Q说易懂点就是风控$0"
        echo -e "$G登录频繁被腾讯限制登录\n只能用手机号或扫码\n但目前没有手机号登录的框架\n扫码只支持手表还不一定成功\n所以237一律换号\n$0"
        ;;
        
    5)
        echo -e "$L235不知道什么原因$0"
        echo -e "$G删了虚拟设备信息就可以了$0"
        cd ~/Yunzai-Bot
if [ -d /root/Yunzai-Bot/data/icqq ]; then
   rm -rf data/icqq
else
