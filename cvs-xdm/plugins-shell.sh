...
echo -e "—————————————————————————————————————————————"
echo -e "\e[1;32m当前插件安装路径:\e[0m\e[33m$Yz\e[0m"
echo -e "—————————————————————————————————————————————
 输入（\e[33m
 全部安装\e[0m）即可安装以下全部插件
 ...
 \e[32m38\e[0m.Py插件    \e[32m39\e[0m.冰析插件   \e[32m40\e[0m.FanSky_Qs
 \e[32m41\e[0m.崩铁插件  \e[32m42\e[0m.阿尔萌插件 \e[32m43\e[0m.阴天插件
—————————————————————————————————————————————"
...
#———————————————————————————————————————————————————————————————
        ...
        clear
if [ -d "$Yz/plugins/Icepray" ]; then
    echo -e "\e[1;32m冰析插件文件已存在 需要重装请先删除.\e[0m"
else
         cd $Yz
	    echo -e "\e[1;32m开始执行冰析插件安装命令.\e[0m"
git clone https://gitee.com/koinori/Icepray.git ./plugins/Icepray 
fi

if [ -d "$Yz/plugins/Icepray" ]; then
    echo -e "\e[1;32m冰析插件安装成功.\e[0m"
else
    echo -e "\e[1;31m冰析插件安装失败.\e[0m"
   sleep 1.6 
fi
        ;;
#———————————————————————————————————————————————————————————————
