...
echo -e "—————————————————————————————————————————————
 ...
 \e[32m44\e[0m.银趴插件  \e[32m45\e[0m.文案插件   \e[32m46\e[0m.寸幼萱插件
 \e[32m47\e[0m.鳄梨插件  \e[32m48\e[0m.憨憨插件   \e[32m49\e[0m.清凉插件
 \e[32m50\e[0m.面板插件   
—————————————————————————————————————————————";
...
#———————————————————————————————————————————————————————————————
        43)
        clear
if [ -d "$Yz/plugins/chatgpt-plugin" ]; then
    echo -e "\e[1;31m ChatGPT 插件文件已存在 需要重装请先删除.\e[0m"
    sleep 3
else
         cd $Yz
	    echo -e "\e[1;32m开始执行 ChatGPT 插件安装命令.\e[0m"
git clone --depth=1 https://ghproxy.com/https://github.com/ikechan8370/chatgpt-plugin.git ./plugins/chatgpt-plugin/
if [ -d "$Yz/plugins/chatgpt-plugin" ]; then
...
#———————————————————————————————————————————————————————————————
        ...
        clear
       cd $Yz
        if [ -d "$Yz/plugins/impart-pro-plugin" ]; then
        echo -e "\e[1;32mimpart-pro-plugin插件文件已存在 需要重装请先删除.\e[0m"
        sleep 3
   else
        echo -e "\e[32m开始执行银趴插件安装命令\e[0m"
...
#———————————————————————————————————————————————————————————————
        ...
        cd $Yz
        if [ -d "$Yz/plugins/y-tian-plugin" ]; then
        echo -e "\e[1;32mwenan-plugin插件文件已存在 需要重装请先删除.\e[0m"
        sleep 3
    else
...
#———————————————————————————————————————————————————————————————
        ...
        clear
        cd $Yz
    if [ -d "$Yz/plugins/avocado-plugin" ]; then
        echo -e "\e[1;32mavocado-plugin插件文件已存在 需要重装请先删除.\e[0m"
    else
        echo -e "\e[32m开始执行鳄梨插件安装命令\e[0m"
...
#———————————————————————————————————————————————————————————————
        ...
        clear
        cd $Yz
    if [ -d "$Yz/plugins/hanhan-plugin" ]; then
        echo -e "\e[1;32mhanhan-plugin插件文件已存在 需要重装请先删除.\e[0m"
    else
        echo -e "\e[32m开始执行憨憨插件安装命令\e[0m"
...
#———————————————————————————————————————————————————————————————
        ...
        clear
        cd $Yz
    if [ -d "$Yz/plugins/ql-plugin" ]; then
        echo -e "\e[1;32mql-plugin插件文件已存在 需要重装请先删除.\e[0m"
    else
        echo -e "\e[32m开始执行清凉插件安装命令\e[0m"
...
#———————————————————————————————————————————————————————————————
        ...
        clear
        cd $Yz
    if [ -d "$Yz/plugins/panel-plugin" ]; then
        echo -e "\e[1;32mpanel-plugin插件文件已存在 需要重装请先删除.\e[0m"
    else
        echo -e "\e[32m开始执行面板插件安装命令\e[0m"
        git clone https://gitee.com/yunzai-panel/panel-plugin ./plugins/panel-plugin/
        if [ -d "$Yz/plugins/panel-plugin" ]; then
        echo -e "\e[32m面板插件安装成功\e[0m"
    else
        echo -e "\e[31m面板插件安装失败，请检查网络\e[0m"
    fi
    fi
    ;;
#———————————————————————————————————————————————————————————————
